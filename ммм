...
pthread_mutex_t counter_mutex; // мьютекс для синхронизации доступа к глобальной переменной
...
void *thread_function(void *arg) {
    ...
    for (i = 0; i < 1000000; i++) {
        pthread_mutex_lock(&counter_mutex); // блокируем мьютекс для доступа к глобальной переменной
        counter++; // увеличиваем значение счетчика на 1
        pthread_mutex_unlock(&counter_mutex); // разблокируем мьютекс
    }
...
int main(int argc, char *argv[]) {
    ...
    pthread_mutex_init(&counter_mutex, NULL); // инициализируем мьютекс
    ...
    pthread_mutex_destroy(&counter_mutex); // удаляем мьютекс
    return 0;
}
